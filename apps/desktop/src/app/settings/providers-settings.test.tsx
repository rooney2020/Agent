...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: /Remove MiniMax|移除 MiniMax/ })
    await act(async () => {
  ...
  it('does not offer removal for externally managed providers', async () => {
    ...
    expect(screen.getByText(/managed by its own CLI|由其自身的 CLI 管理/)).toBeTruthy()
  })
  ...
  it('orders API-key providers by priority then name, and filters them via search', async () => {
    ...
    const search = screen.getByPlaceholderText(/Search providers|搜索提供方/)
    await act(async () => {
    ...
    expect(await screen.findByText(/No providers match your search|没有匹配的提供方/)).toBeTruthy()
  })
  ...
  it('offers a Local / custom endpoint entry in the API-keys tab that opens the custom-endpoint flow', async () => {
    ...
    const row = await screen.findByText(/Local \/ custom endpoint|本地 \/ 自定义端点/)
    expect(screen.getByText(/OpenAI-compatible endpoint|OpenAI 兼容端点/)).toBeTruthy()
