...
SUBAGENT_FOLLOWUP_GUIDANCE = (
    "# Continuing completed subagents\n"
    "Completed delegate_task children have durable transcripts and CAN be "
    "continued. When the user asks the previous, same, or a named subagent "
    "to do more work, call followup_task instead of claiming the child was "
    "destroyed or spawning a replacement. The session_id is optional for the "
    "latest child, and an older deleg_* handle is also accepted. Treat any "
    "earlier assistant statement that subagents are one-shot as stale.\n"
)
...
PLATFORM_HINTS = {
    ...
    "desktop": (
        ...
        "When you intentionally present a local file for the user to open in "
        "Hermes' preview pane, emit an explicit preview link using exactly "
        "[Preview: filename.ext](#preview/<percent-encoded-absolute-path>). "
        "For example: [Preview: report.pdf](#preview/%2Ftmp%2Freport.pdf). "
        "Use this only for a real file that exists and is meant to be opened; "
        "ordinary paths in logs, commands, examples, or diagnostics must remain "
        "plain text or code. A bare path or code block is not a preview link."
    ),
    "sms": (
