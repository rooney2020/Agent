...
import { Box, Check, Pencil, X } from '@/lib/icons'
...
function RegionSelectionOverlay({
  ...
  const attach = () => {
    ...
    const locator = {
      ...
      format: kind,
      path: filePath,
      ...(kind === 'pdf' ? { selectionType: 'image-region' } : {}),
      coordinates: {
        unit: 'percent',
        x: percent(region.x),
        y: percent(region.y),
    ...
    mainComposerScope.add({
      id: `selection:${filePath}:${JSON.stringify(locator.coordinates)}`,
      kind: 'selection',
      label: `${label} · 选区`,
      detail: `${kind.toUpperCase()} · ${locator.coordinates.x}%, ${locator.coordinates.y}%`,
      path: filePath,
      refText
    })
    setActive(false)
  ...
  const annotate = () => {
    ...
    const percent = (value: number) => Math.round(value * 1000) / 10
    const locator = {
      type: 'preview-region', format: kind, path: filePath,
      ...(kind === 'pdf' ? { selectionType: 'image-region' } : {}),
      coordinates: { unit: 'percent', x: percent(region.x), y: percent(region.y), width: percent(region.width), height: percent(region.height) }
    }
  ...
  return (
    <>
      <button
        aria-pressed={active}
        className="absolute right-3 top-3 z-30 inline-flex h-7 items-center gap-1.5 rounded-lg border border-border/70 bg-background/90 px-2.5 text-[0.68rem] font-medium text-foreground shadow-sm backdrop-blur transition-colors hover:bg-accent"
        onClick={() => {
      ...
      >
        ...
        {active ? '退出框选' : kind === 'pdf' ? '框选图片/区域并追问' : '框选并追问'}
      </button>
      {active && (
        <div
          className="absolute inset-0 z-20 cursor-crosshair touch-none bg-primary/[0.03]"
          onPointerDown={event => {
            ...
            setRegion({ ...next, width: 0, height: 0 })
          }}
          onPointerMove={event => {
        ...
        >
          {region && (
            <div
              className="pointer-events-none absolute border-2 border-primary bg-primary/10 shadow-[0_0_0_9999px_rgba(15,23,42,0.12)]"
              style={{
                left: `${region.x * 100}%`,
                top: `${region.y * 100}%`,
          ...
          {region && region.width > 0.01 && region.height > 0.01 && (
            <div className="absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 gap-2" onPointerDown={event => event.stopPropagation()} onPointerMove={event => event.stopPropagation()} onPointerUp={event => event.stopPropagation()}>
              <button className="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground shadow-lg" onClick={event => { event.stopPropagation(); attach() }} type="button">引用此区域</button>
              <button className="rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-semibold text-foreground shadow-lg" onClick={event => { event.stopPropagation(); annotate() }} type="button">添加批注</button>
            </div>
          )}
        </div>
      )}
...
export function LocalFilePreview({ reloadKey, target }: { reloadKey: number; target: PreviewTarget }) {
  ...
  if (isImage && state.dataUrl) {
    return (
      <div className="relative flex h-full w-full items-center justify-center overflow-auto bg-transparent p-4">
        <img
          alt={target.label}
          ...
          src={state.dataUrl}
        />
        <RegionSelectionOverlay filePath={filePath} kind="image" label={target.label} />
      </div>
    )
  }
  ...
  if (isPdf && state.dataUrl && pdfUrl) {
    return (
      <div className="relative h-full w-full overflow-hidden bg-transparent">
        <PdfDocumentPreview dataUrl={state.dataUrl} filePath={filePath} label={target.label} />
        <RegionSelectionOverlay filePath={filePath} kind="pdf" label={target.label} />
      </div>
    )
  }
