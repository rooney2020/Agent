...
test('registry lets a re-open promote an existing spectator window', () => {
  const registry = createSessionWindowRegistry()
  const win = makeFakeWindow()
  let promoted = 0

  registry.openOrFocus('child-session', () => win)
  registry.openOrFocus('child-session', () => win, (existing, sessionId) => {
    assert.equal(existing, win)
    assert.equal(sessionId, 'child-session')
    promoted += 1
  })

  assert.equal(promoted, 1)
  assert.equal(registry.size, 1, 'promotion reuses the existing OS window')
