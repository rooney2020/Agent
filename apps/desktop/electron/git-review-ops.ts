...
// Review paths returned by Git are relative to the repository root, even when
// the active session started in a nested directory. Run every path-sensitive
// review operation from that same root so a row such as `toolsets.py` cannot
// accidentally be interpreted as `<session cwd>/toolsets.py`.
async function repositoryRoot(cwd, gitBin) {
  try {
    const root = (await gitFor(cwd, gitBin).revparse(['--show-toplevel'])).trim()
...
async function reviewList(repoPath, scope, baseRef, gitBin) {
  ...
  try {
    ...
    cwd = await repositoryRoot(cwd, gitBin)
  } catch {
...
async function reviewStage(repoPath, filePath, gitBin) {
  const requestedCwd = resolveRequestedPathForIpc(repoPath, { purpose: 'Review stage' })
  const cwd = await repositoryRoot(requestedCwd, gitBin)
...
async function reviewUnstage(repoPath, filePath, gitBin) {
  const requestedCwd = resolveRequestedPathForIpc(repoPath, { purpose: 'Review unstage' })
  const cwd = await repositoryRoot(requestedCwd, gitBin)
...
async function reviewRevert(repoPath, filePath, gitBin) {
  const requestedCwd = resolveRequestedPathForIpc(repoPath, { purpose: 'Review revert' })
  const cwd = await repositoryRoot(requestedCwd, gitBin)
  const git = gitFor(cwd, gitBin)
...
async function reviewCommit(repoPath, message, push, gitBin) {
  const requestedCwd = resolveRequestedPathForIpc(repoPath, { purpose: 'Review commit' })
  const cwd = await repositoryRoot(requestedCwd, gitBin)
  const git = gitFor(cwd, gitBin)
...
async function reviewPush(repoPath, gitBin) {
  const requestedCwd = resolveRequestedPathForIpc(repoPath, { purpose: 'Review push' })
  const cwd = await repositoryRoot(requestedCwd, gitBin)
  const git = gitFor(cwd, gitBin)
  const status = await git.status()
