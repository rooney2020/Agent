  ...
  return (
    <SettingsContent>
      <div className="space-y-6">
        <section>
          <SectionHeading icon={Globe} meta={`${endpoints.length}`} title="自定义端点" />
          <div className="divide-y divide-border/40 rounded-md border border-border/50">
            {endpoints.length ? (
              ...
              <EmptyState description="在下方添加兼容 OpenAI API 的端点。" title="暂无自定义端点" />
            )}
          </div>
        </section>

        <section>
          <SectionHeading icon={Plus} title={form.id ? '编辑端点' : '添加端点'} />
          <div className="grid gap-3 rounded-md border border-border/50 p-3">
            ...
            <label className="grid gap-1.5 text-xs text-muted-foreground">
              ...
              <Input
                ...
                placeholder={form.id ? '留空则保留当前密钥' : '可选'}
                type="password"
                value={form.apiKey}
              />
            </label>
