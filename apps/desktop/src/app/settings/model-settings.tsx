...
interface ModelSettingsProps {
  ...
  /** Opens the provider-management surface without leaving Settings. */
  onAddProvider?: () => void
}

export function ModelSettings({ onAddProvider, onMainModelChanged }: ModelSettingsProps) {
  const { t } = useI18n()
  ...
  return (
    <div className="grid gap-6">
      <section>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {needsSetup ? (
            ...
            placeholder={`粘贴 ${selectedProviderRow?.key_env ?? 'API Key'}`}
                  type="password"
                  value={apiKeyDraft}
                />
                <Button
              ...
              <Button onClick={startProviderSetup} size="sm" variant="textStrong">
                配置 {selectedProviderRow?.name ?? 'Provider'}
              </Button>
            )
          ) : (
            <>
          ...
          {onAddProvider ? (
            <Button onClick={onAddProvider} size="sm" variant="textStrong">
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ? `${selectedProviderRow?.name} 需要 API Key，配置后才能选择模型。`
              : `${selectedProviderRow?.name} 需要通过浏览器登录，Agent 会引导你完成授权。`}
          </p>
        )}
      ...
      {moa && currentMoaPreset && (
        <section>
          <SectionHeading icon={Cpu} title="多 Agent 协作（MoA）" />
          <p className="mb-2 text-xs text-muted-foreground">
            配置可复用的 MoA 预设。预设会作为模型显示，由聚合模型汇总各参考模型的分析并生成最终回复。
          </p>
          ...
          <div className="mb-2 text-xs text-muted-foreground">
            默认预设：<span className="font-mono">{moa.default_preset}</span>
          </div>
          <div className="grid gap-1">
            {currentMoaPreset.reference_models.map((slot, index) => (
              <ListRow
                action={
                  <Switch
                    aria-label={`${slot.enabled !== false ? '停用' : '启用'}参考模型 ${index + 1}`}
