...
function isBundledOfficeFileTarget(target: PreviewTarget): boolean {
  if (target.kind !== 'file') return false

  return [target.path, target.source, target.url].some(value => (value ? /\.(?:docx|pptx|xlsm|xlsx)(?:$|[?#])/i.test(value) : false))
}
...
export function decodePreviewTabs(raw: string): PreviewTab[] {
  ...
  const tabs = (Array.isArray(parsed) ? parsed.filter(isPreviewTab) : []).map(tab => {
    ...
    if (isBundledOfficeFileTarget(tab.target) && tab.target.previewKind === 'binary') {
      return { ...tab, target: { ...tab.target, previewKind: 'office' as const } }
    }
...
let newPreviewSequence = 0

/** A runtime-only chooser tab; artifact tabs are intentionally not persisted. */
export function openPreviewStart() {
  newPreviewSequence += 1
  openPreview({
    kind: 'artifact',
    label: '新建标签页',
    source: 'preview-start',
    transient: true,
    url: `preview-start:${Date.now()}:${newPreviewSequence}`
  })
}
...
export function closeRightRailTab(tabId: string) {
  ...
  if (next.length === 0 && current[index]?.target.source !== 'preview-start') {
    $previewTabs.set([])
