...
class TestDesktopHintEntry:
    ...
    def test_desktop_hint_requires_explicit_preview_links(self):
        hint = PLATFORM_HINTS["desktop"]

        assert "[Preview: filename.ext](#preview/<percent-encoded-absolute-path>)" in hint
        assert "A bare path or code block is not a preview link" in hint
        assert "real file that exists" in hint
