...
// Keep Chromium's native locale, Accept-Language and first native surfaces in
// sync with the desktop's Simplified Chinese default. The renderer may still
// switch languages later from persisted display.language configuration.
...
if (USER_DATA_OVERRIDE) {
  ...
  // The visible product name is intentionally generic, but existing desktop
  // installs keep their connection tokens, window state and preferences under
  // the legacy user-data directory. Pin that storage identity so rebranding
  // never silently creates a fresh profile or breaks authenticated API calls.
...
if (IS_WINDOWS) {
  ...
  // expose Agent sessions/config to every packaged app on the machine.
...
// always-on timers. A completely idle, minimized Agent burned ~20% CPU
...
// HERMES_HOME — the user-facing root for everything Agent-related. Mirrors
...
// ACTIVE_HERMES_ROOT — the canonical mutable Agent install. Same path
...
// active-profile.json records which Agent profile the desktop launches its
...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Agent'
...
const PREVIEW_OFFICE_EXTENSIONS = new Set([
  '.doc',
  '.docx',
  '.odp',
  '.ods',
...
// Seed the native About panel with the live Agent version. This is refreshed
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 provider Research'
})
...
let bootProgressState = {
  ...
  message: 'Waiting to start Agent backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Agent will start automatically when it completes…',
        12
      )
    },
...
function findSystemPython() {
  ...
  // CPython but several Agent deps (notably pywinpty's Rust-built
...
// `hermes update`). Instead we spawn the staged Agent-Setup binary with
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Agent — this window will close and the updater will open. Don’t reopen Agent yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Agent install open ' +
        '(a second Agent window or a terminal running hermes?). Close it and retry.'
    ...
    // Agent processes running from this venv.  The updater normally refuses
    ...
    if (scriptHandoff) {
      ...
      // abort with "Another Agent update is already running (PID <itself>)" —
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Agent — this window will close. Don’t reopen Agent yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Agent.app/Contents/...`
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Agent at ${ACTIVE_HERMES_ROOT}`,
    command,
    args: ['-m', 'hermes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: HERMES_HOME,
...
function resolveHermesBackend(backendArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Agent source at ${overrideRoot}`, backendArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Agent source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Agent runtime at ${ACTIVE_HERMES_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Agent override under WSL: ${hermesOverride}`)
      }
    } else {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Agent CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (shouldTrustHermesOverride(hermesOverride) || verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        ...
        return {
          label: `existing Agent CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Agent CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Agent install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Agent recovery was handed off to Agent Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Agent install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Agent bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `Agent install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Agent' terminal tool calls bash.exe
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Agent on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Agent venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Agent at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Agent backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          // Keep the backend wire protocol stable when changing visible
          // product branding. The Python gateway authenticates this exact
          // compatibility header on every protected /api route.
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Agent backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Agent backend after ${timeoutMs}ms`))
    })
...
// signal so the chat composer doesn't stay stuck on "Starting Agent...".
...
//
// Hosted Agent gateways gate the dashboard behind an OAuth provider (e.g.
// provider Research) instead of a static session token. The auth model is
...
//   * provider account now issues a 24h ROTATING, reuse-detected refresh token
...
// single-shot boot() with no retry) surfaced as the "Agent couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Agent
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Agent Cloud agent…' : 'Sign in to Agent gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Agent backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Agent Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the provider portal ONCE in
...
// Canonical provider portal base URL, overridable for staging/dev. Mirrors the CLI
// convention (hermes_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Agent surface to the same portal.
...
// Whether the OAuth partition currently holds a live provider portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the Agent gateway session cookies, so this
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Agent Cloud sign-in.'))
...
// Discover the hosted (Agent Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Agent Cloud. Open Settings → Gateway, choose Agent Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Agent Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Agent Cloud session has expired. Sign in to Agent Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Agent Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    remoteUrl,
    // The persisted Agent Cloud org (slug/id) for a cloud connection, or '' for
...
// `org` (optional) is the Agent Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Agent Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    if (
      ...
      const err = new Error(
        'Remote Agent gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Agent gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Agent gateway is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Agent backend.'
      )
    }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Agent
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with provider Research" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Agent on the remote host before connecting with Desktop SSH.'
            }
          }
  ...
  // connect to Agent gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry) {
  ...
  rememberLog(`Starting Agent backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Agent backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Agent backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Agent backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Agent backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Agent backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
async function startHermes() {
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Agent backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote Agent backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Agent backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Agent runtime', 28)
    ...
    await advanceBootProgress('backend.spawn', `Starting Agent backend via ${backend.label}`, 84)
    rememberLog(`Starting Agent backend via ${backend.label}`)
    ...
    if (!processOwner) {
      stopBackendChild(hermesProcess)
      throw new Error('Agent backend start was superseded by a newer connection attempt.')
    }
    ...
    hermesProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Agent backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Agent backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Agent backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Agent backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Agent backend exit (${signal || code})`)
      ...
      rememberLog(`Agent backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Agent backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Agent backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Agent backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Agent backend to become ready', 90)
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Agent backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Agent backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createSessionWindow(sessionId, { watch = false } = {}) {
  return sessionWindows.openOrFocus(
    sessionId,
    () => spawnSecondaryWindow({ sessionId, watch }),
    existing => {
      // A running subagent opens as a cheap, read-only spectator. Once the
      // child is terminal, opening it again must promote that same OS window
      // to a normal resumable chat instead of focusing the stale watch view.
      if (!watch && existing.webContents?.getURL?.().includes('watch=1')) {
        loadWindowUrl(
          existing,
          buildSessionWindowUrl(sessionId, {
            devServer: DEV_SERVER,
...
// here so it can leave the app's bounds and stay visible while Agent is
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Agent app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Agent icon drops out of cmd/alt-tab — especially when the
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Agent in the normal
...
// window showing only the composer and its scrollback, so Agent can be driven
...
// "Starting Agent…". Before the renderer's backoff loop reconnects, it asks us
...
// --- Agent Cloud (cloud-auto-discovery Phase 3) ---
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Agent' TUI into
...
// Resolve the canonical Agent version (the one `release.py` bumps in
// hermes_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Agent version instead of the Electron app's own package.json version,
...
// Re-resolve the live Agent version and push it into the native About panel
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Agent agent venv at ${VENV_ROOT}.`
    }
  }
...
// A preview guest owns its navigation history. Electron 40 handles
// target="_blank" in the guest process and no longer reliably forwards the
// old renderer-side `new-window` event, so route safe web links back into the
// same preview tab here instead of silently discarding the click.
app.on('web-contents-created', (_event, contents) => {
  if (contents.getType() !== 'webview') return

  // Once the user clicks inside a web preview, keyboard focus belongs to this
  // guest WebContents, not the React host. Handle the browser refresh shortcut
  // here so a physical F5 works regardless of which page element is focused.
...
// Seed Chromium's spellchecker with the active locale (falling back to zh-CN).
...
function configureSpellChecker() {
  try {
    ...
    const locale = (app.getLocale && app.getLocale()) || 'zh-CN'
    const candidates = [locale, locale.split('-')[0], 'zh-CN', 'zh']
    const chosen = candidates.find(lang => available.includes(lang))

    // Chromium builds do not all ship a Chinese Hunspell dictionary. Passing
    // an unavailable code throws; an empty list truthfully disables checking
    // instead of silently switching a Chinese UI back to English spelling.
    defaultSession.setSpellCheckerLanguages(chosen ? [chosen] : [])
  } catch (error) {
...
app.on('before-quit', event => {
  ...
  // quitting Agent never keeps another app's chord hostage.
