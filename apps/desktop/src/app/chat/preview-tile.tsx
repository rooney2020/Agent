...
import { $activeTreeGroup, $layoutTree, moveTreePane, revealTreePane } from '@/components/pane-shell/tree/store'
import { FileTypeIcon } from '@/components/ui/file-type-icon'
import { ToolIcon } from '@/components/ui/tool-icon'
import { $rightRailActiveTabId, type RightRailTabId, selectRightRailTab } from '@/store/layout'
import { $previewTabs, closeRightRailTab, openPreviewStart, type PreviewTarget } from '@/store/preview'
...
const TABBED_PREVIEW_MIGRATION_KEY = 'hermes.desktop.previewTabsLayout.v1'

/**
 * Preview files share one default workspace. The first preview opens beside
 * the chat; every later preview joins that preview's tab strip. This only
 * controls first adoption — once a user drags a tab elsewhere, the persisted
 * layout tree keeps that explicit arrangement.
 */
export function defaultPreviewDock(tabId: string): { anchor: string; dir: 'center' | 'right' } {
  const host = $previewTabs.get().find(tab => tab.id !== tabId)

  return host
    ? { anchor: `${PREVIEW_TILE_PREFIX}:${host.id}`, dir: 'center' }
    : { anchor: 'workspace', dir: 'right' }
}

/** Collapse the old "one split per file" default once. After this marker is
 * written, the normal persisted tree is authoritative, so intentional user
 * splits are never folded back automatically. */
function migrateLegacyPreviewSplits(): void {
  if (typeof window === 'undefined' || window.localStorage.getItem(TABBED_PREVIEW_MIGRATION_KEY) === 'done') {
    return
  }
  ...
  if (!tree || tabs.length < 2) {
    return
  }

  const hostPane = `${PREVIEW_TILE_PREFIX}:${tabs[0]!.id}`
  const hostGroup = findGroupOfPane(tree, hostPane)

  if (!hostGroup) {
    return
  }

  // Mark before moving: each move publishes the tree synchronously and would
  // otherwise re-enter this migration through the layout listener.
  window.localStorage.setItem(TABBED_PREVIEW_MIGRATION_KEY, 'done')

  for (const tab of tabs.slice(1)) {
    const paneId = `${PREVIEW_TILE_PREFIX}:${tab.id}`
    const currentTree = $layoutTree.get()
    const currentGroup = currentTree ? findGroupOfPane(currentTree, paneId) : null

    if (currentGroup && currentGroup.id !== hostGroup.id) {
      moveTreePane(paneId, { groupId: hostGroup.id, pos: 'center' })
    }
  }
}
...
export function watchPreviewTiles(): void {
  ...
  migrateLegacyPreviewSplits()
  $previewTabs.listen(migrateLegacyPreviewSplits)
  $layoutTree.listen(migrateLegacyPreviewSplits)
...
const watchPreviewTileMirror = paneMirror<{ id: string }>({
  ...
  // One preview workspace: the first file opens beside main; later files stack
  // into it as tabs. NOT anchored to the file tree — doing that would make
  // ⌘J (toggle file browser) take the preview with it.
  dir: tab => defaultPreviewDock(tab.id).dir,
  anchor: tab => defaultPreviewDock(tab.id).anchor,
  minWidth: '22rem',
  title: previewTitle,
  ...
  stripTools: tabId => [
    ...
    {
      id: 'new-preview-tab',
      icon: <ToolIcon name="add" size="0.75rem" />,
      label: '新建预览标签页',
      onSelect: openPreviewStart
    }
  ],
