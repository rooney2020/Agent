...
export function requestModelOptions({
  ...
  const withoutBrandedProvider = (response: ModelOptionsResponse): ModelOptionsResponse => ({
    ...response,
    providers: (response.providers ?? []).filter(provider => provider.slug !== 'nous')
  })
  ...
  if (gateway) {
    ...
    return gateway.request<ModelOptionsResponse>('model.options', params).then(withoutBrandedProvider)
  }

  return getGlobalModelOptions({ explicitOnly, ...(refresh ? { refresh: true } : {}) }).then(withoutBrandedProvider)
}
