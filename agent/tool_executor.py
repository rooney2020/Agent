...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if function_name == "todo":
        ...
        elif function_name in {"delegate_task", "followup_task"}:
            tasks_arg = function_args.get("tasks")
            if function_name == "followup_task":
                prompt_preview = (function_args.get("prompt") or "")[:30]
                spinner_label = f"↪️ {prompt_preview or 'continuing subagent'}"
            ...
            try:
                def _execute(next_args: dict) -> Any:
                    if function_name == "followup_task":
                        return agent._dispatch_followup_task(next_args)
                ...
                tool_duration = time.time() - tool_start_time
                cute_msg = _get_cute_tool_message_impl(function_name, function_args, tool_duration, result=_delegate_result)
                if spinner:
