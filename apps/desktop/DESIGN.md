...
Notes:
- Boxed buttons use the shared 8px control radius; icon buttons use the same
  radius scale. Inline text/link variants remain visually boxless.
...
- **`controlVariants`** (`src/components/ui/control.ts`) is the shared 8px,
  minimum-32px-high shape for `Input` / `Textarea` / `SelectTrigger`. New
  text-entry controls compose it.
