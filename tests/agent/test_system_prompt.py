...
def test_followup_capability_overrides_stale_one_shot_assumption():
    stable = _stable_prompt(
        _make_agent(valid_tool_names=["followup_task"], platform="desktop")
    )
    assert "Completed delegate_task children" in stable
    assert "CAN be continued" in stable
    assert "earlier assistant statement" in stable
