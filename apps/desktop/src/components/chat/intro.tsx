...
const FALLBACK_COPY: IntroCopy[] = [
  ...
  {
    headline: 'What should Agent look at?',
...
const ZH_INTRO_COPY: IntroCopy[] = [
  { headline: '随时开始', body: '说说你要完成的任务。我会选择合适的工具、说明计划，并在高风险操作前征求你的确认。' },
  { headline: '今天想做什么？', body: '可以给我一个文件、一段报错或一个目标，我会从当前工作区开始调查并验证结果。' },
  { headline: '从任意位置开始', body: '输入仓库路径、异常堆栈或还不完整的想法。我会先理解上下文，再提出可执行的下一步。' },
  { headline: '工作区已就绪', body: '我可以搜索代码、编辑文件、运行测试和整理结果。告诉我你想达到的目标即可。' },
  { headline: '等待你的任务', body: '输入问题、代码片段或任务描述。我会保留会话上下文，并在不确定时向你确认。' }
]
...
function fallbackCopyForPersonality(personalityKey: string): IntroCopy[] {
  ...
  return [
    ...
    {
      headline: `What does ${label} Agent need to see?`,
    ...
    {
      headline: `What should ${label} Agent tackle?`,
...
export function Intro({ personality, seed }: IntroProps) {
  ...
  const copy = locale === 'zh' ? pickCopy(ZH_INTRO_COPY, effectiveSeed) : resolveCopy(personality, effectiveSeed)
