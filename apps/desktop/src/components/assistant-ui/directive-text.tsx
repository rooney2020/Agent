...
const PREVIEW_SELECTION_RE = /\s*<preview_selection>\s*([\s\S]*?)\s*<\/preview_selection>\s*/gi

export function extractPreviewSelections(text: string): { cleanedText: string; selections: PreviewSelectionData[] } {
  const selections: PreviewSelectionData[] = []
  const cleanedText = text.replace(PREVIEW_SELECTION_RE, (_whole, payload: string) => {
    try {
      const parsed = JSON.parse(payload) as PreviewSelectionData

      if (parsed && typeof parsed === 'object') {
        selections.push(parsed)
      }
    } catch {
      // Preserve malformed blocks as ordinary text so context is never hidden.
      return _whole
    }
  ...
  return { cleanedText: cleanedText.trimEnd(), selections }
}

export function appendPreviewSelections(text: string, selections: readonly PreviewSelectionData[]): string {
  const body = text.trimEnd()
  const blocks = selections.map(selection => `<preview_selection>\n${JSON.stringify(selection, null, 2)}\n</preview_selection>`)

  return [body, ...blocks].filter(Boolean).join('\n\n')
}

export function PreviewSelectionCard({
  onRemove,
  selection
}: {
  onRemove?: () => void
  ...
  const filename = selection.label || selection.path?.split(/[\\/]/).filter(Boolean).pop() || '预览选区'
  const format = selection.format?.toUpperCase() || 'FILE'
  const coordinates = selection.coordinates || {}
  const x = typeof coordinates.x === 'number' ? `${coordinates.x}%` : ''
  const y = typeof coordinates.y === 'number' ? `${coordinates.y}%` : ''
  const element = selection.elementTag
    ? `${selection.selectionType === 'image' ? '图片' : selection.elementTag}${typeof selection.elementIndex === 'number' ? ` ${selection.elementIndex + 1}` : ''}`
    : ''
  const slide = typeof selection.slide === 'number' ? `第 ${selection.slide} 页` : typeof selection.page === 'number' ? `第 ${selection.page} 页` : ''
  const annotationCount = Array.isArray(selection.annotations) ? `${selection.annotations.length} 条批注` : ''
  const summary = [format, slide, selection.shapeName, selection.sheet, selection.range, element, annotationCount, x && y ? `${x}, ${y}` : '']
    .filter(Boolean)
    .join(' · ')

  return (
    <span
      className="my-1.5 block max-w-xl overflow-hidden rounded-lg border border-(--ui-stroke-tertiary) bg-[color-mix(in_srgb,currentColor_4%,transparent)] text-[0.7rem]"
      data-slot="preview-selection-card"
    >
      <span
        aria-expanded={open}
        className="flex w-full items-center gap-2 px-2.5 py-2 text-left transition-colors hover:bg-[color-mix(in_srgb,currentColor_5%,transparent)]"
        onKeyDown={event => {
      ...
      >
        <span className="grid size-5 shrink-0 place-items-center rounded bg-primary/10 text-primary">
          <DirectiveIcon className="size-3" type="file" />
        </span>
        <span className="min-w-0 flex-1">
          <span className="block truncate font-medium text-foreground">{filename} · 选区</span>
          <span className="block truncate text-muted-foreground">{summary}</span>
        </span>
        {onRemove && (
          <button
            aria-label="移除选取信息"
            className="grid size-5 shrink-0 place-items-center rounded text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
            onClick={event => {
        ...
        <span aria-hidden className={cn('text-muted-foreground transition-transform', open && 'rotate-90')}>›</span>
      </span>
      {open && (
        <pre className="max-h-48 overflow-auto border-t border-(--ui-stroke-tertiary) px-2.5 py-2 font-mono text-[0.66rem] leading-relaxed text-muted-foreground">
          {JSON.stringify(selection, null, 2)}
        </pre>
      )}
    </span>
...
export function DirectiveContent({ text }: { text: string }) {
  const previewContent = useMemo(() => extractPreviewSelections(text ?? ''), [text])
  const { cleanedText, images } = useMemo(
    () => safeEmbeddedImages(previewContent.cleanedText),
  ...
  return (
    <span className="whitespace-pre-line" data-slot="aui_directive-text">
      ...
      {previewContent.selections.map((selection, index) => (
        <PreviewSelectionCard key={`${selection.path || 'selection'}-${index}`} selection={selection} />
      ))}
    </span>
  )
}
