...
export function TitlebarControls({ leftTools = [], tools = [], onOpenSettings }: TitlebarControlsProps) {
  ...
  const previewPaneIds = previewTabs.map(tab => `preview-tile:${tab.id}`)
  const previewVisible = previewPaneIds.some(id => !hiddenTreePanes.has(id))
  ...
  const previewTool: TitlebarTool = {
    ...
    icon: <TitlebarIcon name={previewVisible ? 'layout-panel-off' : 'layout-panel'} />,
    id: 'preview-window',
    label: previewVisible ? t.titlebar.hidePreview : t.titlebar.showPreview,
    onSelect: () => {
      triggerHaptic('tap')

      if (previewPaneIds.length > 0) {
        for (const paneId of previewPaneIds) {
          setTreePaneHidden(paneId, previewVisible)
        }

        if (!previewVisible) {
          revealTreePane(previewPaneIds.at(-1)!)
        }
