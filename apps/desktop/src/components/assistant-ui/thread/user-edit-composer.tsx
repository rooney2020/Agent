...
import {
  appendPreviewSelections,
  extractPreviewSelections,
  hermesDirectiveFormatter,
  PreviewSelectionCard,
  type PreviewSelectionData
...
export const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const previewSelectionsRef = useRef<PreviewSelectionData[]>([])
  const [previewSelections, setPreviewSelections] = useState<PreviewSelectionData[]>([])
  ...
  const appendExternalText = useCallback(
    (text: string, mode: ComposerInsertMode) => {
      ...
      if (editor) {
        renderComposerContents(editor, extractPreviewSelections(next).cleanedText, { trailingCommitted: true })
        placeCaretEnd(editor)
      }
  ...
  useEffect(() => {
    ...
    if (
      ...
      const previewContent = extractPreviewSelections(draft)

      previewSelectionsRef.current = previewContent.selections
      setPreviewSelections(previewContent.selections)
      renderComposerContents(editor, previewContent.cleanedText, { trailingCommitted: true })
  ...
  const syncDraftFromEditor = useCallback(
    (editor: HTMLDivElement) => {
      const visibleText = sanitizeComposerInput(composerPlainText(editor))
      const nextDraft = appendPreviewSelections(visibleText, previewSelectionsRef.current)
  ...
  const removePreviewSelection = useCallback(
    (index: number) => {
      rememberInitialDraft()
      const nextSelections = previewSelectionsRef.current.filter((_, itemIndex) => itemIndex !== index)
      const visibleText = editorRef.current
        ? sanitizeComposerInput(composerPlainText(editorRef.current))
        : extractPreviewSelections(draftRef.current).cleanedText
      const nextDraft = appendPreviewSelections(visibleText, nextSelections)

      previewSelectionsRef.current = nextSelections
      ...
      <StickyHumanMessageContainer>
        ...
        >
          ...
          <div
            ...
            data-slot="aui_edit-composer-surface"
          >
            {previewSelections.length > 0 && (
              <div className="aui-edit-preview-selections" data-slot="aui_edit-preview-selections">
                {previewSelections.map((selection, index) => (
                  <PreviewSelectionCard
                    key={`${selection.path || selection.label || 'selection'}-${index}`}
                    onRemove={() => removePreviewSelection(index)}
                    selection={selection}
                  />
                ))}
              </div>
            ...
            <div
              ...
              autoCorrect="off"
              className={cn(
                'ui-prompt-input-editor__input max-h-48 min-w-0 w-full resize-none overflow-x-hidden bg-transparent text-[length:var(--conversation-text-font-size)] text-foreground/95 outline-none',
