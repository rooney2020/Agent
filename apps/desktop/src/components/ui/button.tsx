...
const buttonVariants = cva(
  "inline-flex shrink-0 cursor-pointer items-center justify-center gap-1.5 rounded-lg text-xs leading-4 font-medium whitespace-nowrap transition-[color,background-color,border-color,box-shadow,transform] duration-100 outline-none focus-visible:ring-[0.1875rem] focus-visible:ring-primary/20 disabled:pointer-events-none disabled:cursor-default disabled:opacity-50 active:translate-y-px aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        ...
        outline:
          'border border-(--ui-stroke-secondary) bg-transparent text-(--ui-text-primary) shadow-xs hover:border-primary/30 hover:bg-(--chrome-action-hover) hover:text-(--ui-text-primary)',
        // Soft-fill action (the default "non-primary button" look).
        secondary:
          'border border-(--ui-stroke-tertiary) bg-(--ui-bg-quaternary) text-(--ui-text-primary) shadow-xs hover:bg-(--chrome-action-hover) hover:text-(--ui-text-primary)',
