...
describe('Office previews', () => {
  it('keeps Office files on their original path for the bundled renderer', () => {
    expect(localPreviewTarget('/tmp/spec.docx')).toMatchObject({
      path: '/tmp/spec.docx',
      previewKind: 'office',
      url: 'file:///tmp/spec.docx'
    })
    expect(localPreviewTarget('/tmp/slides.pptx')).toMatchObject({ previewKind: 'office' })
    expect(localPreviewTarget('/tmp/table.xlsx')).toMatchObject({ previewKind: 'office' })
    expect(localPreviewTarget('/tmp/macro-enabled.xlsm')).toMatchObject({ previewKind: 'office' })
  })
})
