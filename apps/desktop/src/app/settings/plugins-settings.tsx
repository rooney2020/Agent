...
function localizedAgentPluginDescription(row: AgentPluginRow): string | undefined {
  const description = row.description?.trim()

  if (!description || /[\u3400-\u9fff]/.test(description)) {
    return description || (row.version ? `版本 ${row.version}` : undefined)
  }

  const key = (row.key ?? '').toLowerCase()

  if (key.startsWith('browser/')) return `${row.name} 浏览器自动化后端。启用后可供 Agent 执行网页操作。`
  if (key.startsWith('image_gen/')) return `${row.name} 图像生成后端。启用后可供 Agent 生成或编辑图片。`
  if (key.startsWith('video_gen/')) return `${row.name} 视频生成后端。启用后可供 Agent 生成或编辑视频。`
  if (key.startsWith('web/')) return `${row.name} 网页搜索与内容提取插件。`
  if (key.startsWith('observability/')) return `${row.name} 可观测性插件，用于记录和分析 Agent 运行数据。`
  if (key.startsWith('cron/')) return `${row.name} 定时任务插件。`
  if (key.startsWith('integrations/')) return `${row.name} 第三方服务集成插件。`
  if (key.startsWith('security/')) return `${row.name} 安全检查与风险提示插件。`

  return `${row.name} 功能扩展插件。`
}
...
function AgentPluginRowView({ row }: { row: AgentPluginRow }) {
  ...
  return (
    <PluginLine
      ...
      description={localizedAgentPluginDescription(row)}
      title={
        <>
          <span>{row.name}</span>
