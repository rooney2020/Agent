...
test('review diff uses repository-root paths when the session cwd is nested', async () => {
  const dir = makeRepo()
  const nested = path.join(dir, 'packages', 'desktop')

  fs.mkdirSync(nested, { recursive: true })
  fs.writeFileSync(path.join(dir, 'tracked.txt'), 'tracked\nchanged from repository root\n')

  const result = await reviewList(nested, 'uncommitted', null, 'git')
  const file = result.files.find(entry => entry.path === 'tracked.txt')

  assert.ok(file)

  const diff = await reviewDiff(nested, file.path, 'uncommitted', null, file.staged, 'git')

  assert.match(String(diff), /\+changed from repository root/)
})
