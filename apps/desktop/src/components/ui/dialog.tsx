...
function DialogOverlay({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Overlay>) {
  return (
    <DialogPrimitive.Overlay
      className={cn(
        'fixed inset-0 z-(--z-modal-backdrop) pointer-events-auto bg-slate-950/35 backdrop-blur-sm data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0',
        className
      )}
...
function DialogContent({
  ...
  const closeButton = showCloseButton ? (
    <DialogPrimitive.Close
      ...
      className="absolute right-2.5 top-2.5 z-20 inline-flex size-8 touch-manipulation items-center justify-center rounded-lg text-(--ui-text-tertiary) transition-[color,background-color,box-shadow] hover:bg-(--chrome-action-hover) hover:text-foreground focus-visible:ring-[0.1875rem] focus-visible:ring-primary/20 focus-visible:outline-none"
      ...
      <DialogPortal>
        ...
        <DialogPrimitive.Content
          className={cn(
            ...
            'fixed left-1/2 top-1/2 z-(--z-modal) pointer-events-auto flex max-h-[88vh] -translate-x-1/2 -translate-y-1/2 flex-col rounded-2xl bg-[color-mix(in_srgb,var(--ui-bg-elevated)_94%,transparent)] text-[length:var(--conversation-text-font-size)] text-foreground shadow-nous backdrop-blur-xl duration-150 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0',
            widthClass,
            className,
        ...
        >
          <DialogPortalContainerContext.Provider value={contentNode}>
            ...
            <div className="relative z-10 overflow-hidden rounded-2xl border border-b-0 border-(--stroke-nous) bg-[color-mix(in_srgb,var(--ui-bg-elevated)_96%,transparent)]">
              <div className={cn('grid max-h-[calc(88vh-5rem)] min-h-0 gap-4 overflow-y-auto p-6', bodyClassName)}>
                {children}
              </div>
            </div>
            <div
    ...
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        className={cn(
          ...
          'fixed left-1/2 top-1/2 z-(--z-modal) pointer-events-auto flex max-h-[88vh] -translate-x-1/2 -translate-y-1/2 flex-col rounded-2xl border border-(--stroke-nous) bg-[color-mix(in_srgb,var(--ui-bg-elevated)_94%,transparent)] text-[length:var(--conversation-text-font-size)] text-foreground shadow-nous backdrop-blur-xl duration-150 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0',
          widthClass,
          className
        )}
        data-slot="dialog-content"
      ...
      >
        <DialogPortalContainerContext.Provider value={contentNode}>
          ...
          <div className={cn('grid min-h-0 gap-4 overflow-y-auto rounded-[inherit] p-6', bodyClassName)}>
            {children}
          </div>
          {closeButton}
        </DialogPortalContainerContext.Provider>
