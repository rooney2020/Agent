...
class TestDelegateRequirements(unittest.TestCase):
    ...
    def test_followup_schema_requires_child_and_prompt(self):
        self.assertEqual(FOLLOWUP_TASK_SCHEMA["name"], "followup_task")
        self.assertEqual(
            FOLLOWUP_TASK_SCHEMA["parameters"]["required"],
            ["prompt"],
        )
...
class TestFollowupTask(unittest.TestCase):
    def test_delegation_handle_resolves_to_latest_owned_child(self):
        parent = _make_mock_parent()
        ...
        parent._session_db.get_session.side_effect = [None, child_row]
        parent._session_db.list_sessions_rich.return_value = [child_row]
        parent._session_db.get_messages_as_conversation.return_value = [
        ...
        with (
            ...
            patch(
                ...
                return_value={"status": "completed", "summary": "continued"},
            ),
        ):
            result = json.loads(
                followup_task("deleg_old", "next", parent_agent=parent)
            )

        self.assertEqual(result["session_id"], "child-session")

    def test_omitted_session_selects_latest_owned_child(self):
        parent = _make_mock_parent()
        parent.session_id = "parent"
        parent._session_db = MagicMock()
        parent._session_db.list_sessions_rich.return_value = [
            {
                "id": "latest-child",
                "parent_session_id": "parent",
                "model_config": {"_delegate_from": "parent"},
        ...
        parent._session_db.get_session.return_value = {
            "id": "latest-child",
            "parent_session_id": "parent",
            "model_config": {"_delegate_from": "parent"},
        }
        ...
        with (
            ...
            result = json.loads(followup_task(None, "next", parent_agent=parent))

        self.assertEqual(result["session_id"], "latest-child")
        self.assertEqual(build.call_args.kwargs["session_id"], "latest-child")

    def test_rejects_a_child_owned_by_another_conversation(self):
        parent = _make_mock_parent()
        ...
        parent._session_db.get_conversation_root.side_effect = lambda sid: sid

        result = json.loads(
            followup_task("child", "continue", parent_agent=parent)
        )

        self.assertIn("not a subagent owned", result["error"])

    def test_resumes_the_same_child_with_its_persisted_history(self):
        parent = _make_mock_parent()
        parent.session_id = "parent"
        parent._session_db = MagicMock()
        history = [
        ...
        parent._session_db.get_messages_as_conversation.return_value = history
        child = MagicMock()
        ...
        with (
            ...
            patch(
                "tools.delegate_tool._run_single_child",
                return_value={"status": "completed", "summary": "next result"},
            ) as run,
        ):
            result = json.loads(
                followup_task("child", "next task", parent_agent=parent)
            )

        self.assertTrue(result["continued"])
        self.assertEqual(result["session_id"], "child")
        self.assertIs(child._delegate_conversation_history, history)
        self.assertEqual(build.call_args.kwargs["session_id"], "child")
        run.assert_called_once_with(0, "next task", child, parent)
