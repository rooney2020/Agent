...
DELEGATE_BLOCKED_TOOLS = frozenset(
    [
        ...
        "followup_task",  # no cross-branch continuation from a child
    ...
    try:
        ...
        def _run_with_thread_capture():
            ...
            with delegated_child_context(str(getattr(child, "session_id", "") or "")):
                ...
                resumed_history = vars(child).get(
                    "_delegate_conversation_history"
                )
                if resumed_history is not None:
                    run_kwargs["conversation_history"] = resumed_history
...
def _delegate_parent_owns_session(parent_agent, record: Dict[str, Any]) -> bool:
    """Return whether *record* is a delegate child of this conversation."""
    parent_sid = str(getattr(parent_agent, "session_id", "") or "")
    if not parent_sid:
        return False
    config = record.get("model_config") or {}
    if isinstance(config, str):
        try:
            config = json.loads(config)
        except (TypeError, ValueError):
            config = {}
    delegate_from = str(config.get("_delegate_from") or "")
    direct_parent = str(record.get("parent_session_id") or "")
    if parent_sid in {delegate_from, direct_parent}:
        return True

    # Compression may rotate either side of the relationship. Compare durable
    # conversation roots so a legitimate child remains follow-up capable.
    db = getattr(parent_agent, "_session_db", None)
    get_root = getattr(db, "get_conversation_root", None)
    if not callable(get_root):
        return False
    try:
        parent_root = str(get_root(parent_sid) or parent_sid)
        owner = delegate_from or direct_parent
        return bool(owner) and str(get_root(owner) or owner) == parent_root
    except Exception:
        return False


def followup_task(session_id: str, prompt: str, parent_agent=None) -> str:
    """Continue a durable delegate child session from its owning parent."""
    sid = str(session_id or "").strip()
    text = str(prompt or "").strip()
    if parent_agent is None:
        return tool_error("followup_task requires a parent agent context.")
    if not text:
        return tool_error("prompt is required.")

    db = getattr(parent_agent, "_session_db", None)
    if db is None:
        return tool_error("No durable session store is available.")
    if not sid:
        try:
            ...
            owned = [
                ...
                and _delegate_parent_owns_session(parent_agent, row)
            ]
            if owned:
                sid = str(owned[0].get("id") or "").strip()
        except Exception as exc:
            return tool_error(f"Could not locate the previous subagent: {exc}")
        if not sid:
            return tool_error("This conversation has no previous subagent to continue.")

    try:
        record = db.get_session(sid)
    except Exception as exc:
        return tool_error(f"Could not load subagent session {sid}: {exc}")
    # Async delegate_task returns a delegation handle (deleg_*) immediately.
    # Older completion messages did not expose the durable child session id,
    # so models naturally pass that visible handle back here. Resolve it to
    # the latest owned child instead of reporting the durable session missing.
    if not isinstance(record, dict) and sid.startswith("deleg_"):
        try:
            candidates = db.list_sessions_rich(
                limit=500,
                include_children=True,
            ...
            return tool_error(f"Could not resolve delegation handle: {exc}")
    if not isinstance(record, dict):
        return tool_error(f"Subagent session not found: {sid}")
    if not _delegate_parent_owns_session(parent_agent, record):
        return tool_error(
            "The requested session is not a subagent owned by this conversation."
        )
    try:
        history = db.get_messages_as_conversation(sid, repair_alternation=True)
    except Exception as exc:
        return tool_error(f"Could not load subagent transcript {sid}: {exc}")
    if not history:
        return tool_error(f"Subagent session has no resumable transcript: {sid}")
    ...
    child = _build_child_preserving_parent_tools(
        ...
        goal=text,
        context=(
            "Continue the existing delegated conversation using its persisted "
            "transcript. This is a follow-up from the owning parent Agent; do "
            "not claim that completed subagents cannot be continued."
        ),
        toolsets=None,
        model=creds["model"],
        max_iterations=max_iterations,
    ...
    child._delegate_conversation_history = history
...
FOLLOWUP_TASK_SCHEMA = {
    "name": "followup_task",
    "description": (
        "Continue an existing completed subagent in the same durable child "
        "conversation. Use this when the user asks the previous/same subagent "
        "to do more work; do not spawn a replacement with delegate_task."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "session_id": {
                "type": "string",
                "description": (
                    "Optional child session_id. Omit it when the user says "
                    "the previous/latest subagent; the most recent child "
                    "owned by this conversation is selected automatically."
                ),
            },
            "prompt": {
                "type": "string",
                "description": "The follow-up instruction for that same subagent.",
            },
        },
        "required": ["prompt"],
    },
...
registry.register(
    name="followup_task",
    toolset="delegation_followup",
    schema=FOLLOWUP_TASK_SCHEMA,
    handler=lambda args, **kw: followup_task(
