...
describe('preview store', () => {
  ...
  it('falls back to a neighbouring tab when the active one closes, and clears the selection on the last', () => {
    ...
    expect($previewTarget.get()?.source).toBe('preview-start')
    expect($rightRailActiveTabId.get()).toContain('preview-start:')
  })
  ...
  it('closes by the raw source the composer rows were handed', () => {
    ...
    expect($previewTabs.get()).toHaveLength(1)
    expect($previewTabs.get()[0]?.target.source).toBe('preview-start')
  ...
  it('migrates a persisted macro-enabled workbook from binary to the bundled Office renderer', () => {
    const target = {
      binary: true,
      kind: 'file',
      label: '详细设计.xlsm',
      path: '/work/详细设计.xlsm',
      previewKind: 'binary',
      source: '/work/详细设计.xlsm',
      url: 'file:///work/%E8%AF%A6%E7%BB%86%E8%AE%BE%E8%AE%A1.xlsm'
    }
    const [tab] = decodePreviewTabs(JSON.stringify([{ id: `file:${target.url}`, target }]))

    expect(tab.target.previewKind).toBe('office')
  })
