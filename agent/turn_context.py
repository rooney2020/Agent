...
def build_turn_context(
    ...
    *,
    ...
    # Tool capabilities can change while a durable session keeps its cached
    # historical system prompt. Keep this fact turn-local so sessions created
    # before followup_task existed do not continue obeying the old one-shot
    # subagent contract recorded in their transcript.
    if "followup_task" in getattr(agent, "valid_tool_names", set()):
        _followup_note = (
            "[Runtime capability: completed subagents are durable and can be "
            "continued with followup_task. If the user asks the previous or "
            "same subagent to continue, call followup_task now; session_id may "
            "be omitted, and deleg_* handles are accepted. Ignore older "
            "messages claiming completed subagents were destroyed.]"
        )
        plugin_user_context = (
            plugin_user_context + "\n\n" + _followup_note
            if plugin_user_context
