...
function ZoneMenu({
  ...
  /** Preview workspaces keep their file tabs visible so content can never strand them. */
...
export function TreeGroup({
  ...
  const maximizedGroupId = useStore($maximizedTreeGroupId)
  ...
  const hasPreviewPane = shown.some(id => id.startsWith('preview-tile:'))
  const previewMaximized = hasPreviewPane && maximizedGroupId === node.id

  useEffect(() => {
    if (!previewMaximized) return
    const restore = (event: KeyboardEvent) => {
      if (event.key === 'Escape') $maximizedTreeGroupId.set(null)
    }
  ...
  const headerHidden =
    paneChrome(active).headerVeto || (!hasPreviewPane && (node.headerHidden ?? (shown.length <= 1 && !forceLoneHeader)))
  ...
  // Drag handles preventDefault pointerdown, so native dblclick never fires.
  // Keep the synthesized double-tap only to restore a collapsed group. Hiding
  // the tab bar is an explicit context-menu action; an accidental double-click
  // must never make the navigation chrome disappear.
  const headerDoubleTap: DoubleTapContext = {
    key: `restore-header-${node.id}`,
    onDoubleTap: () => setTreeGroupMinimized(node.id, false)
  }
  ...
  return (
    <div
      className={cn(
        ...
        // Tree content intentionally scopes --titlebar-height to zero. Use the
        // app's fixed 34px native-titlebar clearance here so Linux/Windows
        // window controls never cover the preview strip's restore button.
        previewMaximized && 'fixed inset-x-0 bottom-0 top-[34px] z-80'
      )}
      ...
      {headerVisible && (
        <ZoneMenu {...zoneMenu}>
          ...
          >
              ...
              const tab = (
                <PaneTab
                  ...
                  onPointerDown={e => {
                    ...
                    if (!chrome.tabDrag?.(e, onTap, headerDoubleTap)) {
                      startPaneDrag(
                        paneId,
                        e,
                        onTap,
            ...
            {hasPreviewPane && !node.minimized && (
              <PaneStripGlyph
                icon={<Codicon name={previewMaximized ? 'screen-normal' : 'screen-full'} size="0.8125rem" />}
                label={previewMaximized ? '恢复预览区' : '最大化预览区'}
                onSelect={() => $maximizedTreeGroupId.set(previewMaximized ? null : node.id)}
              />
            )}
