...
export function SegmentedControl<T extends string>({
  ...
  return (
    <div
      className={cn(
        'inline-grid w-fit auto-cols-fr grid-flow-col gap-1 rounded-xl border border-(--ui-stroke-tertiary) bg-(--ui-bg-tertiary) p-1 shadow-xs',
        disabled && 'opacity-50',
        className
      )}
    >
      {options.map(({ id, label, icon: Icon }) => {
        ...
        return (
          <button
            aria-pressed={active}
            className={cn(
              'flex min-h-7 items-center justify-center gap-1 rounded-lg px-3 py-1 text-[0.6875rem] font-medium transition-[color,background-color,box-shadow] disabled:cursor-default',
              active
